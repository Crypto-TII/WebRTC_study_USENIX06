...
public class NoCertRequestBypass extends AuthBypass {

    public NoCertRequestBypass() {
        super("Ignore client, handshake with a server without presenting certificate", false, true);
    }

    /**
     * Creates a MitM trace based on the assumption that the client does not check the certificate
     * and the server does not request a certificate. The client is left at the CH.
     *
     * <pre>
     *
     * Client                    MitM                  Server
