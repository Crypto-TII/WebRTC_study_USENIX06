...
public class SrtcpEncryptingProcessor extends EncryptionProcessor<RtpCrypto, ByteArrayBuffer> {

    public SrtcpEncryptingProcessor(RtpCrypto cryptoContext) {
        super(cryptoContext);
    }
    ...
    protected ByteArrayBuffer encrypt(RtpCrypto cryptoContext, ByteArrayBuffer plaintext) {
        ...
        if (errEncrypt != SrtpErrorStatus.OK) {
            throw new RuntimeException("Failed SRTCP encryption: " + errEncrypt.name());
        }
