...
public class NoCertRequestBogusServerCertBypass extends AuthBypass {

    public NoCertRequestBogusServerCertBypass() {
        super(
                "present unexpected arbitrary certificate to client, handshake with a server without presenting certificate",
                true,
                true);
    }
     ...
     * and the server does not request a certificate.
     *
     * <pre>
     *
     * Client                    MitM                  Server
    ...
    public boolean isApplicable(WebRtcPlatformReport report) {
        ...
        if (!isFalse(report, WebRtcProperties.SERVER_REQUESTS_CERTIFICATE)) {
            return false;
        }
