...
public class SrtpDecryptingProcessor extends DecryptionProcessor<RtpCrypto, ByteArrayBuffer> {

    public SrtpDecryptingProcessor(RtpCrypto cryptoContext) {
        super(cryptoContext);
    }
    ...
    protected ByteArrayBuffer decrypt(RtpCrypto cryptoContext, ByteArrayBuffer ciphertext) {
        ...
        if (errDecrypt != SrtpErrorStatus.OK) {
            throw new RuntimeException("Failed SRTP decryption: " + errDecrypt.name());
        }
        return ciphertext;
    }
