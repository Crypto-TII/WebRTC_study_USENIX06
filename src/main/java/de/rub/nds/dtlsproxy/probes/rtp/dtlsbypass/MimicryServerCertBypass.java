...
public class MimicryServerCertBypass extends MimicryCertBypass {

    public MimicryServerCertBypass() {
        super("Present mimicked server certificate to client, ignore server", true, false, false);
    }

    /**
     * Creates a trace based on the assumption that the client accepts certificates not matching the
     * SDP fingerprinting, but looking alike the original. The server is left at the CH
     *
     * <pre>
     *
     * Client                    MitM                  Server
